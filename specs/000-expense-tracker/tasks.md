...
- [ ] T067 [US1] Create app/layouts/default.vue with header, navigation (Dashboard, Transactions, Settings, Logout), main content area - Design with `/ui-ux-pro-max` command
- [ ] T068 [US1] Add responsive navigation in app/layouts/default.vue (hamburger menu for mobile, horizontal nav for desktop) - Design with `/ui-ux-pro-max` command
- [ ] T069 [US1] Create app/pages/transactions/index.vue with TransactionList and "Add Transaction" button - Design with `/ui-ux-pro-max` command
- [ ] T070 [US1] Connect TransactionForm to store actions (create/update) in app/pages/transactions/index.vue
- [ ] T071 [US1] Add delete confirmation dialog in TransactionCard.vue ("Xóa giao dịch này?" with amount, category, date) - Design with `/ui-ux-pro-max` command
- [ ] T072 [US1] Implement undo/restore functionality for delete (5 second timeout with toast notification)
- [ ] T073 [US1] Style all transaction components with TailwindCSS (responsive mobile/desktop breakpoints)

### Real-time & Sync

- [ ] T074 [US1] Setup Supabase real-time subscription in app/stores/transactions.ts (listen to INSERT, UPDATE, DELETE events)
- [ ] T075 [US1] Handle subscription events: auto-insert new transactions, update existing, remove deleted (with smooth animations)
- [ ] T076 [US1] Add subscription disconnect handling (fallback to manual refresh, show warning indicator)
...
- [ ] T086 [P] [US2] Create app/composables/useDateRange.ts with period calculations (Daily, Weekly, Monthly)
- [ ] T087 [P] [US2] Implement getPeriodRange(type, offset) function in useDateRange.ts (returns { start, end } dates)
- [ ] T088 [P] [US2] Implement formatPeriodLabel(type, date) function in useDateRange.ts (e.g., "12 Dec 2025", "Week 50, 2025", "Dec 2025")
- [ ] T089 [P] [US2] Add isCurrentPeriod(type, date) check function in useDateRange.ts
- [ ] T090 [P] [US2] Add period navigation functions in useDateRange.ts (goToNext, goToPrevious, goToCurrent)
...
- [ ] T097 [P] [US2] Add getters to app/stores/transactions.ts: getTransactionsByDateRange(start, end)
- [ ] T098 [P] [US2] Add getters to app/stores/transactions.ts: getTotalIncome(start, end), getTotalExpense(start, end)
- [ ] T099 [P] [US2] Add computed balance getter in app/stores/transactions.ts (income - expense)

### Page Implementation

- [ ] T100 [US2] Create app/pages/index.vue (Dashboard page) with 3-column grid layout for summary cards - Design with `/ui-ux-pro-max` command
...
- [ ] T103 [US2] Implement summary calculation in app/pages/index.vue using store getters and date range
- [ ] T104 [US2] Add StatCard components to app/pages/index.vue (Income: green, Expense: red, Balance: green/red/gray)
- [ ] T105 [US2] Add number formatting in app/pages/index.vue using formatCurrency composable
- [ ] T106 [US2] Implement color coding for balance (positive: green, negative: red, zero: gray)
- [ ] T107 [US2] Add empty state UI in app/pages/index.vue ("Chưa có giao dịch nào trong [period]" with CTA button) - Design with `/ui-ux-pro-max` command
- [ ] T108 [US2] Add loading state (skeleton cards) in app/pages/index.vue - Design with `/ui-ux-pro-max` command
- [ ] T109 [US2] Implement responsive layout in app/pages/index.vue (mobile: stacked, tablet/desktop: horizontal row) - Design with `/ui-ux-pro-max` command
